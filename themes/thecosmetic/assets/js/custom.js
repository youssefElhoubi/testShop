...
$(document).ready(function(){   
   ...
   if (length_cat > 10){
    ...
    if ( $('.nav_title_tab').length > 0 ){
        $('.nav_title_tab > li:first-child').find('a').addClass("active");
        var defaultActive = $('.nav_title_tab > li:first-child a.active').attr("href");
        $('.tab_content').find(defaultActive).addClass('active');
    }
    $(document).on('click','.nav_title_tab a:not(.active)',function(e){
        e.preventDefault();
        $('.nav_title_tab a.active').removeClass('active');
        $(this).addClass('active');
        var tabtActive = $(this).attr("href");
        $('.tab_content_block').removeClass('active');
        $('.tab_content').find(tabtActive).addClass('active');
    });
});
